...
void loop() {
  ...
  if (lastSensorTime == 0 || now - lastSensorTime >= SENSOR_INTERVAL_MS) {
    ...
    float lightPct = constrain((ldrRaw / 4095.0f) * 100.0f, 0.0f, 100.0f);

    Serial.printf("Temp: %.1f C | Humidity: %.1f%% | Soil: %.1f%% | pH: %.2f | Light: %.1f%%\n",
                  temperature, humidity, soilMoisture, ph, lightPct);
